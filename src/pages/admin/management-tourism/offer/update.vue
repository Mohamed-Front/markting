        ...
        <div>
          <FileUpload id="password" @uploader="uploadFile" accept="image/*" required="true" multiple="true"
            ...
            <template #empty>
              ...
              <img  v-for="u in url " :src="u" class="w-[200px] h-[90px]  mb-3">
              <img :src="`http://marketing.mqawilk.com/public/images/Products/` + image" >
              <img v-for="data in dataEntred?.media" :src="data.original_url"
...
const cproduct=()=>{
  ...
  axios
    .post(`api/Products/update/${route.params.id}`,body)
    .then((res) => {
      fet()
      toast.add({severity: 'success', summary: 'Successful', detail: 'product created Successful', life: 3000})
    })
    .catch((er) => {
      console.log(er)
...
const fet=()=>{
  axios
    .get(`/api/Offers/One/Offer/${route.params.id}`)
    .then((res) => {

      name.value=res.data.data.name
      old_price.value=res.data.data.old_price
      new_price.value=res.data.data.new_price
